...
if (!empty($_SESSION["login"]) && !empty($_SESSION["pass"]) && $_SESSION["godMode"] == 1) {
  $arr = getNotConfirmed();
  if ($arr !== "") {
    $table = "<table id='confirmContent'><tr><td>Nazwa odbiorcy</td><td>Numer rachunku bankowego</td><td>Adres odbiorcy</td><td>Tytuł przelewu</td><td>Data przelewu</td><td>Kwota</td><td>Akcja</td></tr>";
    foreach ($arr as $row) {
